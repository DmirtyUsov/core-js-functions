...
function getPolynom(...args) {
  return function temp(x) {
    let sum = null;
    const maxDegree = args.length - 1;
    if (maxDegree > -1) {
      sum = 0;
      args.forEach((constant, idx) => {
        sum += constant * x ** (maxDegree - idx);
      });
    }
    return sum;
  };
...
function logger(func, logFunc) {
  return function temp(...args) {
    const argsForLog = args.map((arg) =>
      Array.isArray(arg) ? JSON.stringify(arg) : arg
    );
    const logString = `${func.name}(${argsForLog})`;
